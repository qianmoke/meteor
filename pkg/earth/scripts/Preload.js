(function(module, exports, __webpack_require__) {
	...
	*/
	...
	var Preload = function Preload(pm, keyW) {
	    var that = this;
	    ...
	    var texturesLen = pm.textures ? pm.textures.length : 0;

	    var keyimgsWeight = keyW || 7;
	    var totalLen = imgsLen + spritesLen + keyimgsLen * keyimgsWeight + ajaxsLen + texturesLen;
	    var count = 0;
	    ...
	    var loadTexture = function loadTexture() {
	        ...
	        var successFn = function successFn(texture, name) {
	            count++;
	            texture.name = name;
	            Preload.buffer.textures[name] = texture; // 存入内存中
	            if (count === totalLen) {
	        ...
	        var _loop = function _loop(i) {
	            ...
	            var texture = new _three.TextureLoader();

	            texture.load(pm.textures[i].url, function (texture) {
	                successFn(texture, pm.textures[i].name);
	            });

	            texture.name = pm.textures[i].name;
	        };

	        for (var i = 0; i < texturesLen; i++) {
	            _loop(i);
	        }
	    };
	    ...
	    var loadImg = function loadImg() {
	        var imgSuccessFn = function imgSuccessFn() {
	            count++;
	    ...
	    // 载入帧动画图片
	    var loadKeyimg = function loadKeyimg() {
	        var keyimgSuccessFn = function keyimgSuccessFn() {
	        ...
	        var keyimgLoadFn = function keyimgLoadFn(p) {
	    ...
	    var loadAjax = function loadAjax() {
	        // 计数器
	        var ajaxSuccessFn = function ajaxSuccessFn() {
	            count++;
	            if (count === totalLen) {
	                that.onloading && that.onloading(100);
	                that.onload && that.onload();
	...
	*/
	...
	Preload.loadKeyImgs = function () {
	    ...
	    var fn = function fn(el, pathPrefix, pad, postfix) {
	        var that = this;
